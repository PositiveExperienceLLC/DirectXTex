...
int dxtConvMain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[])
{
    ...
    if (FAILED(hr))
    {
        ...
        {
            wprintf(L"Com already initializd", hr);
    ...
    for (auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv)
    {
        ...
        if (dwOptions & ((DWORD64(1) << OPT_HFLIP) | (DWORD64(1) << OPT_VFLIP)))
        {
            ...
            // AJF: If you are both flipping and resizing an image then this assert is not valid.
                ...
                case DXGI_FORMAT_BC7_UNORM_SRGB:
...

                    {
                        ...
                        if (!s_tryonce)
                        {
                            ...
                            {
                                if (false) //!CreateDevice(adapter, pDevice.GetAddressOf()))
